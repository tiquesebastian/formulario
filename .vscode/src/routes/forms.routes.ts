...
formRouter.get('/:id', async (req, res, next) => {
  try {
    ...
    if (!idParsed.success) {
      return res.status(400).json({ message: idParsed.error.issues[0]?.message ?? 'ID inválido' })
    }
...
formRouter.put('/:id', async (req, res, next) => {
  try {
    ...
    if (!bodyParsed.success) {
      return res.status(400).json({ message: 'Datos inválidos', errors: bodyParsed.error.flatten() })
    }

    if (!bodyParsed.data.status && !bodyParsed.data.data) {
      return res.status(400).json({ message: 'Envía al menos status o data para actualizar' })
    }
